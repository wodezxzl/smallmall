<template>
  <tab-bar>
    <tab-bar-item path="/home">
      <template v-slot:icon>
        <img src="~assets/img/tabbar/home.svg" alt="" />
      </template>
      <template v-slot:active-icon>
        <img src="~assets/img/tabbar/home_active.svg" alt="" />
      </template>
      <template v-slot:word><div>首页</div></template>
    </tab-bar-item>
    <tab-bar-item path="/category">
      <template v-slot:icon>
        <img src="~assets/img/tabbar/category.svg" alt="" />
      </template>
      <template v-slot:active-icon>
        <img src="~assets/img/tabbar/category_active.svg" alt="" />
      </template>
      <template v-slot:word><div>分类</div></template>
    </tab-bar-item>
    <tab-bar-item path="/cart">
      <template v-slot:icon>
        <img src="~assets/img/tabbar/cart.svg" alt="" />
      </template>
      <template v-slot:active-icon>
        <img src="~assets/img/tabbar/cart_active.svg" alt="" />
      </template>
      <template v-slot:word><div>购物车</div></template>
    </tab-bar-item>
    <tab-bar-item path="/profile">
      <template v-slot:icon>
        <img src="~assets/img/tabbar/profile.svg" alt="" />
      </template>
      <template v-slot:active-icon>
        <img src="~assets/img/tabbar/profile_active.svg" alt="" />
      </template>
      <template v-slot:word><div>我的</div></template>
    </tab-bar-item>
  </tab-bar>
</template>

<script>
  import TabBar from '../common/tabbar/TabBar'
  import TabBarItem from '../common/tabbar/TabBarItem'

  export default {
    name: 'MainTabBar',
    components: {
      TabBar,
      TabBarItem,
    },
  }
</script>

<style scoped lang="less"></style>
